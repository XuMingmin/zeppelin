...
public class RheosSojournerTable extends BeamKafkaTable {
  protected RheosSojournerTable(BeamRecordSqlType beamSqlRowType, String bootstrapServers, List<String> topics) {
    super(beamSqlRowType, bootstrapServers, topics);
  }
  
  public static RheosSojournerTable create(String bootstrapServers, String consumerName, String clientId,
      String topicName){
    ...
    return (RheosSojournerTable) new RheosSojournerTable(extractRowType(), bootstrapServers, Arrays.asList(topicName))
  ...
  public static class DBStreamingRecordParser extends DoFn<KV<byte[], byte[]>, BeamRecord>{
    ...
    public void setup(){
      ...
      this.deserializerHelper = new SchemaRegistryAwareAvroDeserializerHelper<GenericRecord>(config, GenericRecord.class);
    }
    ...
    public void onEachElement(ProcessContext context){
      try {
        ...
        GenericRecord avroRecord = deserializerHelper.deserializeData(rhsEvent.getSchemaId(), context.element().getValue());

        
        context.output(new BeamRecord(beamSqlRowType
            , String.valueOf(avroRecord.get("guid"))
            , String.valueOf(avroRecord.get("cguid"))
            , new Date(Long.valueOf(((GenericRecord)avroRecord.get("rheosHeader")).get("eventCreateTimestamp").toString()))
            , avroRecord.get("pageId")
            , String.valueOf(avroRecord.get("siteId"))
